...
# Universe of 40 stocks across 11 GICS sectors
UNIVERSE = [
...
def run():
    ...
    for idx, stock in enumerate(UNIVERSE):
        ...
        # Base mock calculations for testing / pipeline stability
        rev_cur = round((total - idx) * 10.0 + 20.0, 2)
        rev_nxt = round(rev_cur * (1.10 + (idx % 4) * 0.04), 2)

        eps_cur = round(2.50 + (idx % 6) * 1.10, 2)
        eps_nxt = round(eps_cur * (1.12 + (idx % 5) * 0.03), 2)

        pe_fwd = round(15.0 + (idx % 8) * 2.5, 2)

        # Enforce strict "Year" values
        forecast_data.extend([
            {"Symbol": sym, "Sector": sec, "Metric": "Revenue Avg", "Year": "Current Year", "Value": rev_cur},
            {"Symbol": sym, "Sector": sec, "Metric": "Revenue Avg", "Year": "Next Year", "Value": rev_nxt},
            {"Symbol": sym, "Sector": sec, "Metric": "EPS Avg", "Year": "Current Year", "Value": eps_cur},
            {"Symbol": sym, "Sector": sec, "Metric": "EPS Avg", "Year": "Next Year", "Value": eps_nxt},
            {"Symbol": sym, "Sector": sec, "Metric": "Forward PE", "Year": "Current Year", "Value": pe_fwd}
        ])
    ...
    print("Data successfully generated without empty fields.")
